# main.py
# Versione di DEBUG ESTREMO per ispezionare la struttura dei dati in arrivo.
...
def main(ctx: AutomationContext) -> None:
    ...
    """
    print("--- AVVIO SCRIPT DI ISPEZIONE DATI ---", flush=True)
    ...
    try:
        ...
        if not elements:
            print("ERRORE: Non è stata trovata una lista '@elements' nell'oggetto radice.", flush=True)
            ctx.mark_run_failed("La struttura del commit non è quella attesa. Manca la lista '@elements'.")
            return

        print(f"Trovati {len(elements)} elementi nella lista '@elements'.", flush=True)
        ...
        if parameters:
            ...
            print(f"L'oggetto 'parameters' è di tipo: {type(parameters)}", flush=True)
            
            # Controlliamo se 'parameters' è un oggetto con proprietà
        ...
        # Se qualcosa va storto, vogliamo sapere cosa.
