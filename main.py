# main.py
# VERSIONE 14.1 - FUSIONE STABILE (v13.5 + Lettura Parametri Robusta)
...
# ⬇️⬇️⬇️ MOTORE DI LETTURA ROBUSTO DALLA v13.7 ⬇️⬇️⬇️
...
def get_instance_parameter_value(element, group_name: str, param_name: str):
    ...
    return get_parameter_value_from_speckle_object(element, path)
# ⬆️⬆️⬆️ FINE MOTORE DI LETTURA ROBUSTO ⬆️⬆️⬆️
...
def get_ai_suggestion(prompt: str) -> str:
    ...
    try:
        ...
        if model_cost <= 0.1: return '{"is_consistent": false, "suggestion": 45.50, "justification": "Costo unitario non compilato o pari a zero."}'
...
#============== FUNZIONI DELLE REGOLE (ora usano il nuovo motore di lettura) ===========
...
def run_ai_cost_check(elements: list, price_list: list) -> list:
    ...
    for el in elements:
        ...
        try: model_cost = float(get_instance_parameter_value(el, GRUPPO_TESTO, COST_UNIT_PARAM_NAME))
...
#============== ORCHESTRATORE PRINCIPALE (dalla v13.5) =======================
def main(ctx: AutomationContext) -> None:
    print("--- STARTING GOLDEN MASTER VALIDATOR (v14.1 FUSIONE) ---", flush=True)
    try:
        price_list = []
